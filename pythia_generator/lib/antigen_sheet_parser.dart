...
class AntigenSheetParser {
  ...
  Series _parseSeriesRows(List<List<String>> rows) {
    // We'll accumulate data in a mutable local Series, but it's declared final in Pythia,
    ...
    for (var i = 0; i < rows.length; i++) {
      ...
      // 4.g) Age lines for current dose
        ...
        if (row.length > 1 &&
          ...
          final vaxAge = VaxAge(
            ...
            effectiveDate:
                _nullIfNA(row.length > 6 ? row[6] : '')?.simplifyDate,
            cessationDate:
                _nullIfNA(row.length > 7 ? row[7] : '')?.simplifyDate,
          );
      ...
      // 4.h) Preferable Interval
        ...
        if (row.length > 1 &&
          ...
          final newInterval = Interval(
            ...
            effectiveDate:
                _nullIfNA(row.length > 10 ? row[10] : '')?.simplifyDate,
            cessationDate:
                _nullIfNA(row.length > 11 ? row[11] : '')?.simplifyDate,
          );
          if (!newInterval.isEmpty()) {
            ...
            currentDose =
                currentDose.copyWith(preferableInterval: updatedPrefInt);
            doseList[doseList.length - 1] = currentDose;
          }
        }
      }
      ...
      // 4.j) Preferable Vaccine
        ...
        if (!vaccineType.contains('Vaccine Type') &&
          ...
          currentDose =
              currentDose.copyWith(preferableVaccine: existingPrefVac);
          doseList[doseList.length - 1] = currentDose;
        }
      }
      ...
      // 4.l) Inadvertent Vaccine
        ...
        if (!vaccineType.contains('Vaccine Type') &&
          ...
          currentDose =
              currentDose.copyWith(inadvertentVaccine: existingInvVac);
          doseList[doseList.length - 1] = currentDose;
        }
      }
